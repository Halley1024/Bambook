...
pub(crate) fn create<D: NativeDevice>(device: D) -> Result<Device, Error> {
    const {
        // MSVC's bindgen output does not expose max_align_t. The x64 Windows
        // allocator guarantees 16-byte alignment for fundamental allocations.
        #[cfg(windows)]
        assert!(align_of::<D>() <= 16);
        #[cfg(not(windows))]
